...
public class Status extends WeiboResponse implements java.io.Serializable {
    ...
    private List<PicUrl> picUrls = new ArrayList<PicUrl>(); // 图片 url 列表
    ...
    private void constructJson(JSONObject json) throws WeiboException {
        try {
            ...
            if (!json.isNull("pic_urls")) {
                picUrls = PicUrl.fromJSONArray(json.getJSONArray("pic_urls"));
            }
    ...
    public static class PicUrl {
        ...
        public static PicUrl fromJSONObject(JSONObject json) throws JSONException {
            PicUrl picUrl = new PicUrl();
            picUrl.setThumbnailPic(json.getString("thumbnail_pic"));
            return picUrl;
        }

        public static List<PicUrl> fromJSONArray(JSONArray jsonArray) throws JSONException {
            List<PicUrl> picUrls = new ArrayList<PicUrl>();
            for (int i = 0; i < jsonArray.length(); i++) {
                picUrls.add(PicUrl.fromJSONObject(jsonArray.getJSONObject(i)));
            }
            return picUrls;
        }
